...
namespace :deploy do
  ...
  # FIXME: Starting job of delayed jobs gemz
  desc "Starting job of delayed jobs gem"
  task :start_delayed_jobs do
    #Commenting following lines until it gets fixed -Fabian
    #run "cd #{current_path} && #{sudo} RAILS_ENV=#{rails_env} script/delayed_job stop"
    #puts "\n\n*** DELAYED JOBS STOPPED\n\n"
    #run "cd #{current_path} && #{sudo} RAILS_ENV=#{rails_env} script/delayed_job start"
    #puts "\n\n *** DELAYED JOBS STARTED\n\n"
  end
